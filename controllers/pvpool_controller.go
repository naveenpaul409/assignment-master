...
const (
	...
	DECOMMISSIONING_STATE = "Decommissioning"
...
func checkPodIndDecommissionState(newStatus *pvpoolv1.PvPoolStatus) bool {
	for _, pInfo := range newStatus.PodsInfo {
		if pInfo.PodStatus == DECOMMISSIONING_STATE || pInfo.PodStatus == DECOMMISSIONED_STATE {
			return true
		}
	}
	return false
...
func (r *PvPoolReconciler) reconcilePvPoolStatefulset(pvp *pvpoolv1.PvPool, sts *appsv1.StatefulSet, srv *corev1.Service, newStatus *pvpoolv1.PvPoolStatus, list *corev1.PodList) error {
	...
	shouldUpdate := false
	podDifference := 0
	podIndDecommissionState := checkPodIndDecommissionState(newStatus)
	if podIndDecommissionState {
		r.Log.Info("One or more pods are in decommission state")
	}
	if pvp.Spec.NumPVs > *sts.Spec.Replicas {
		shouldUpdate = !podIndDecommissionState
		...
		shouldUpdate = true
		podDifference = int(*sts.Spec.Replicas - pvp.Spec.NumPVs)
		// set the number of replicas to the number of PVs
		sts.Spec.Replicas = &pvp.Spec.NumPVs
		// set the phase to "Scaling"
		newStatus.Phase = pvpoolv1.PvPoolPhaseScaling
	...
	if podDifference > 0 {
		for i := 0; i < podDifference; i++ {
			pod := list.Items[i]
			r.decommissionStorageAgent(r.getPodURL(pod.Name, pod.Spec.Subdomain, pod.Namespace))
		}
	}
...
func (r *PvPoolReconciler) collectPodsStatus(pvpStatus *pvpoolv1.PvPoolStatus, list *corev1.PodList) error {
	...
	for _, pod := range list.Items {
		...
		if err != nil {
			...
			podStorageUsed = int(agentStatus.Used)
		}
		pvpStatus.PodsInfo = append(pvpStatus.PodsInfo,
			pvpoolv1.PvPodSInfo{PodName: pod.Name, PodStatus: state, PodStorageUsed: podStorageUsed})
